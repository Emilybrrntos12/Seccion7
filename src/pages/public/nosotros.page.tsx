...
export const Nosotros: React.FC = () => (
  <>
    <Header />
    <section className="bg-[#f5ecd7] py-12 px-4">
      <div className="max-w-5xl mx-auto">
        <h1 className="text-3xl md:text-4xl font-bold text-[#8d6748] mb-4 text-center">Nosotros</h1>
        ...
        <div className="bg-white rounded-xl shadow-md p-8 mb-10">
          <h2 className="text-2xl font-bold text-[#8d6748] mb-2">Nuestra historia</h2>
          ...
          <h3 className="text-xl font-semibold text-[#8d6748] mb-2">Nuestros valores</h3>
          <ul className="list-disc pl-6 text-[#5d4037] mb-4">
        ...
        <div className="mb-10">
          <h2 className="text-2xl font-bold text-[#8d6748] mb-6 text-center">Nuestro equipo artesanal</h2>
          <div className="flex flex-wrap justify-center gap-8">
            {artesanos.map(a => (
              <div key={a.nombre} className="w-56 bg-white rounded-xl shadow p-4 flex flex-col items-center">
                <img src={a.foto} alt={a.nombre} className="w-28 h-28 object-cover rounded-full mb-3 border-4 border-[#c2a77d]" />
                <p className="font-bold text-[#8d6748] text-lg">{a.nombre}</p>
                <p className="text-[#5d4037] text-sm">{a.rol}</p>
              </div>
            ))}
          </div>
        </div>
        <div>
          <h2 className="text-2xl font-bold text-[#8d6748] mb-6 text-center">Galería del proceso artesanal</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6">
            {galeria.map((img, idx) => (
              <div key={idx} className="bg-white rounded-xl shadow p-2 flex flex-col items-center">
                <img src={img.src} alt={img.alt} className="w-full h-40 object-cover rounded mb-2" />
                <p className="text-[#5d4037] text-sm text-center">{img.alt}</p>
              </div>
            ))}
          </div>
        </div>
